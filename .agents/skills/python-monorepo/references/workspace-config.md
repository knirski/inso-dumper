...
```bash
# Root project and its dependency closure
uv sync
...
### Locking applications (For production)

```toml
dependencies = ["pydantic>=2,<3"]
```

Commit `uv.lock` and use `uv sync --locked` for selected application versions. Do not use exact published dependency metadata pins merely to reproduce an application environment.
...
See [the skill overview](../SKILL.md) for structure, Docker, and namespace-package references.
